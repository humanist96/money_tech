...
async function getDashboardData() {
  try {
    const [channelsRes, videosRes, statsRes, videoCountRes] = await Promise.all([
      supabase
        .from("channels")
        .select("*")
    ...
    const videos = (videosRes.data ?? []) as unknown as VideoWithChannel[]
    const stats = (statsRes.data ?? []) as DailyStat[]
    const totalVideos = videoCountRes.count ?? 0

  // Today's videos count
