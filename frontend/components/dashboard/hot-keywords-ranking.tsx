  ...
  if (keywords.length === 0) {
    return (
      <div className="glass-card-elevated rounded-2xl p-6">
        ...
        <p className="text-sm text-[#5a6a88]">키워드 데이터를 수집 중입니다.</p>
      </div>
    )
  }
    ...
    <div className="glass-card-elevated rounded-2xl overflow-hidden">
      ...
      <div className="p-3 space-y-0.5">
        {keywords.map((kw, i) => {
          ...
          const changeColor = isNew ? "#00e8b8" : isUp ? "#22c997" : isDown ? "#ff5757" : "#5a6a88"

          return (
            <div key={kw.keyword} className="flex items-center gap-3 px-3 py-2 rounded-xl hover:bg-[#0e1a30]/50 transition">
              <span className="text-[11px] font-bold tabular-nums w-5 text-center" style={{ fontFamily: 'var(--font-outfit)', color: i < 3 ? '#00e8b8' : '#5a6a88' }}>
                {i + 1}
              </span>
              <span className="flex-1 text-sm text-white font-medium truncate">{kw.keyword}</span>
              <span className="text-[10px] tabular-nums text-[#5a6a88]" style={{ fontFamily: 'var(--font-outfit)' }}>
                {kw.count}회
              </span>
              <div className="w-12 flex items-center justify-end gap-0.5" style={{ color: changeColor }}>
                {isNew ? (
                  <span className="text-[9px] font-bold px-1.5 py-0.5 rounded bg-[#00e8b8]/10 text-[#00e8b8]">NEW</span>
                ) : kw.rank_change !== 0 ? (
                  <>
