  ...
  if (data.length === 0) {
    return (
      <div className="glass-card-elevated rounded-2xl p-6">
        ...
        <p className="text-sm text-th-dim">활동 데이터를 수집 중입니다.</p>
      </div>
    )
  }
    ...
    <div className="glass-card-elevated rounded-2xl overflow-hidden">
      ...
      <div className="p-4 overflow-x-auto">
        <table className="w-full text-xs">
          <thead>
            <tr>
              <th className="text-left py-2 px-2 text-th-dim font-medium text-[10px] uppercase tracking-wider sticky left-0 bg-th-card z-10 min-w-[100px]">채널</th>
              {dates.map((date) => (
                <th key={date} className="py-2 px-1 text-th-dim font-medium text-center text-[10px] min-w-[40px]">
          ...
          <tbody>
            {channels.map((channel) => (
              <tr key={channel} className="border-t border-th-border/20">
                <td className="py-1.5 px-2 text-th-primary font-medium whitespace-nowrap sticky left-0 bg-th-card z-10 text-[11px]">{channel}</td>
                {dates.map((date) => {
                  ...
                  return (
                    <td key={date} className="py-1.5 px-1 text-center">
                      <div
                        ...
                        style={{
                          background: count > 0 ? `color-mix(in srgb, #00e8b8 ${Math.round(intensity * 50)}%, var(--th-bg-tertiary))` : 'var(--th-bg-tertiary)',
                          color: count > 0 ? '#00e8b8' : 'var(--th-border)',
